...
class BackendStats:
    ...
    def process(self):
        while True:
            varnish_stats = self._get_varnish_stats()
            varnish_stats['process'] = self._get_process_stats()
    ...
    def _get_process_stats(self):
        stdin, stdout, stderr  = self.ssh.exec_command('cat /var/run/varnishd.pid')
        pid = stdout.readline()
        stdin, stdout, stderr = self.ssh.exec_command('top -b -n 1 -d 1 -U nobody | grep varnishd')
    ...
    def _get_varnish_stats(self):
        stdin, stdout, stderr = self.ssh.exec_command(self.varnish_command)
